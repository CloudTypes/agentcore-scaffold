...
class AgentCoreMemoryQuery:
    ...
    def get_memories(
        self, user_id: str, session_id: Optional[str] = None, strategy: Optional[str] = None, max_results: int = 100
    ) -> Dict:
        """
        ...
        """
        results = {"user_id": user_id, "session_id": session_id, "timestamp": datetime.utcnow().isoformat(), "memories": {}}
        ...
        for strat_name in strategies_to_query:
            ...
            if strat_name == "session":
                namespace = strategy_config["namespace_pattern"].format(user_id=user_id, session_id=session_id)
            else:
    ...
    def test_permissions(self) -> Dict[str, bool]:
        ...
        permissions = {"list_memory_records": False, "retrieve_memory_records": False, "get_memory": False}

        # Test 1: ListMemoryRecords
        try:
            print("  Testing bedrock-agentcore:ListMemoryRecords...")
            self.client.list_memory_records(memoryId=self.memory_id, namespace="/summaries/test", maxResults=1)
        ...
        try:
            ...
            self.client.retrieve_memory_records(
                memoryId=self.memory_id, namespace="/semantic/test", searchCriteria={"searchQuery": "test", "topK": 1}
            )
            ...
            if error_code == "ValidationException":
                ...
                try:
                    response = self.client.retrieve_memory_records(
                        memoryId=self.memory_id,
                        namespace=namespace,
                        searchCriteria={"searchQuery": "memory", "topK": max_results},
                    )
    ...
    def search_memories(self, user_id: str, query_text: str, session_id: Optional[str] = None, max_results: int = 10) -> Dict:
        """
        ...
        try:
            response = self.client.retrieve_memory_records(
                memoryId=self.memory_id, namespace=namespace, searchCriteria={"searchQuery": query_text, "topK": max_results}
            )
            ...
            return {"query": query_text, "error": str(e), "error_code": e.response["Error"]["Code"]}
    ...
    def diagnose_namespace(self, user_id: str, session_id: Optional[str] = None) -> Dict:
        ...
        try:
            response = self.client.list_memory_records(memoryId=self.memory_id, namespace=parent_ns, maxResults=10)
            records = response.get("memoryRecordSummaries", response.get("memoryRecords", []))
            # Add namespace to each record (since it's not in the response)
            for record in records:
                if "namespace" not in record:
            ...
            error_code = e.response.get("Error", {}).get("Code", "")
            error_msg = e.response.get("Error", {}).get("Message", "")
            diagnostics["checks"]["parent_namespace"] = {"error": error_msg, "error_code": error_code}
        ...
        if session_id:
            ...
            try:
                response = self.client.list_memory_records(memoryId=self.memory_id, namespace=exact_ns, maxResults=10)
                records = response.get("memoryRecordSummaries", response.get("memoryRecords", []))
                # Add namespace to each record (since it's not in the response)
                for record in records:
                    if "namespace" not in record:
                ...
                error_code = e.response.get("Error", {}).get("Code", "")
                error_msg = e.response.get("Error", {}).get("Message", "")
                diagnostics["checks"]["exact_namespace"] = {"error": error_msg, "error_code": error_code}
                print(f"   ❌ Error: {error_code} - {error_msg}")
        ...
        try:
            response = self.client.list_memory_records(memoryId=self.memory_id, namespace=semantic_ns, maxResults=10)
            records = response.get("memoryRecordSummaries", response.get("memoryRecords", []))
            ...
            error_code = e.response.get("Error", {}).get("Code", "")
            error_msg = e.response.get("Error", {}).get("Message", "")
            diagnostics["checks"]["semantic_namespace"] = {"error": error_msg, "error_code": error_code}
            print(f"   ❌ Error: {error_code} - {error_msg}")
        ...
        try:
            response = self.client.list_memory_records(memoryId=self.memory_id, namespace=prefs_ns, maxResults=10)
            records = response.get("memoryRecordSummaries", response.get("memoryRecords", []))
            # Add namespace to each record
            for record in records:
                if "namespace" not in record:
            ...
            error_code = e.response.get("Error", {}).get("Code", "")
            error_msg = e.response.get("Error", {}).get("Message", "")
            diagnostics["checks"]["preferences_namespace"] = {"error": error_msg, "error_code": error_code}
            print(f"   ❌ Error: {error_code} - {error_msg}")
        ...
        try:
            ...
            response = self.client.list_memory_records(
                memoryId=self.memory_id, namespace=test_ns, maxResults=10  # Get more to see actual records
            )
            records = response.get("memoryRecordSummaries", response.get("memoryRecords", []))
            # Add namespace to each record
            for record in records:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Query and retrieve AWS Bedrock AgentCore Memory records")
    parser.add_argument("--memory-id", default="voice_agent_memory-yupt8b5dkN", help="AgentCore Memory ID")
    parser.add_argument("--region", default="us-west-2", help="AWS region")
    parser.add_argument("--user-id", required=True, help="User identifier (e.g., nathan_cloudtypes_io)")
    parser.add_argument("--session-id", help="Optional session ID for session-specific queries")
    parser.add_argument("--strategy", choices=["session", "semantic", "preferences"], help="Query specific strategy only")
    parser.add_argument("--search", help="Search query text for semantic search")
    parser.add_argument("--max-results", type=int, default=100, help="Maximum results per strategy")
    ...
    parser.add_argument("--test-permissions", action="store_true", help="Test IAM permissions for AgentCore Memory operations")
    parser.add_argument("--verbose", "-v", action="store_true", help="Enable verbose debugging output")
    parser.add_argument("--diagnose", action="store_true", help="Run diagnostic checks to see what namespaces contain records")

    args = parser.parse_args()

    # Initialize query client
    query_client = AgentCoreMemoryQuery(memory_id=args.memory_id, region=args.region, verbose=args.verbose)

    # Run diagnostics if requested
    if args.diagnose:
        diagnostics = query_client.diagnose_namespace(user_id=args.user_id, session_id=args.session_id)
        if args.format == "json":
    ...
    if args.list_sessions:
        ...
        results = query_client.search_memories(
            user_id=args.user_id, query_text=args.search, session_id=args.session_id, max_results=args.max_results
        )
        ...
        results = query_client.get_memories(
            user_id=args.user_id, session_id=args.session_id, strategy=args.strategy, max_results=args.max_results
        )
