...
IMPORTANT: The credentials generated by this script are valid for 12 hours and MUST be
updated in your .env file for each local development session. These temporary credentials
are required for all AWS API calls (Bedrock, AgentCore Memory, etc.) during local development
when using MFA-protected AWS accounts.

Usage:
    1. Run: python scripts/fix_mfa.py
    2. Copy the export commands from the output
    3. Update AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, AWS_SESSION_TOKEN,
       AWS_DEFAULT_REGION, and AWS_ACCOUNT_ID in your .env file
    4. Comment out AWS_PROFILE in your .env file when using session tokens
"""
...
def save_credentials_to_cache(credentials, expiry_time):
    ...
    cache_data = {"credentials": credentials, "expiry": expiry_time.isoformat()}
...
def authenticate_with_mfa():
    ...
    try:
        ...
        # Calculate expiry time (typically 1 hour for assumed roles, but may be up to 12 hours
        # depending on AWS account configuration)
        # Note: Actual credential validity is determined by AWS account settings
