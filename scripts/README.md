...
3. **Environment Variables** (optional, in `.env` file):
   - `AWS_REGION` or `AGENTCORE_MEMORY_REGION`: AWS region for memory operations (default: `us-east-1`)
...
**What it does**:
- Attempts to retrieve session summary using `ListMemoryRecords` API (no semantic search required)
- Falls back to semantic search with multiple query terms if direct listing fails
- Tries parent namespace if exact namespace returns no records
...
**What it does**:
- Lists all records in the specified namespace prefix
...
**What it does**:
- Tests `ListMemoryRecords` API on multiple namespace levels
- Tries semantic search with various query terms
...
1. Set `AWS_REGION` or `AGENTCORE_MEMORY_REGION` in your `.env` file:
   ```
