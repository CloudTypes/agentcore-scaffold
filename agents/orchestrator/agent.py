...
If unclear, respond with 'orchestrator' to handle directly."""
    ...
    async def process(self, request: AgentRequest) -> AgentResponse:
        ...
        try:
            ...
            if specialist == "orchestrator":
                ...
                response_content = await self._route_to_specialist(specialist, request, context)
    ...
    async def _load_context(self, request: AgentRequest) -> List[Dict[str, str]]:
        ...
        # Get recent history
        recent = await self.memory.get_recent_messages(user_id=request.user_id, session_id=request.session_id, limit=10)

        # Get relevant semantic context
        relevant = await self.memory.semantic_search(user_id=request.user_id, query=request.message, limit=5)

        # Combine with request context
        return request.context + relevant + recent

    async def _classify_intent(self, message: str, context: List[Dict[str, str]]) -> str:
        ...
        response = await self.strands_agent.run(messages=[{"role": "user", "content": classification_prompt}])
    ...
    async def _route_to_specialist(self, specialist: str, request: AgentRequest, context: List[Dict[str, str]]) -> str:
    ...
    async def _handle_directly(self, request: AgentRequest, context: List[Dict[str, str]]) -> str:
