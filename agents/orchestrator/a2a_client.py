...
class A2AClient:
    ...
    def _build_message(self, parts: List[Dict], **kwargs) -> Dict:
        ...
        """
        message = {"messageId": self._generate_message_id(), "role": ROLE_USER, "parts": parts}
    ...
    def _build_jsonrpc_request(self, message: Dict, **kwargs) -> Dict:
        ...
        return {"jsonrpc": JSONRPC_VERSION, "method": METHOD_MESSAGE_SEND, "params": params, "id": self._get_next_id()}
    ...
    def _create_media_part(self, media_content: Dict) -> Dict:
        ...
        if media_type == "image":
            ...
            if "s3Location" in source:
                ...
                return {"type": "file", "fileUri": s3_uri, "mimeType": f"image/{format_str}"}
                ...
                return {
                    ...
                    "data": {"base64": base64_data},  # Wrap in dict as A2A server expects
                }
            ...
            if "s3Location" in source:
                # Use FilePart for S3 URIs
                s3_uri = source["s3Location"].get("uri")
                return {"type": "file", "fileUri": s3_uri, "mimeType": f"video/{format_str}"}
            elif "base64" in source:
                # Use DataPart for base64 content
                base64_data = source["base64"]
    ...
    async def call_agent(self, agent_name: str, task: str, **kwargs) -> str:
        """
        ...
        try:
            ...
            response = await self.client.post(endpoint, json=request, headers={"Content-Type": "application/json"})
    ...
    def _extract_response_content(self, result) -> str:
        ...
        if isinstance(result, dict):
            ...
            logger.warning(
                f"Could not extract text from A2A response, returning string representation. Keys: {list(result.keys())}"
            )
            return str(result)
    ...
    async def call_agent_with_media(self, agent_name: str, task: str, media_content: Dict, **kwargs) -> str:
        """
        ...
        try:
            ...
            response = await self.client.post(
                endpoint, json=request, headers={"Content-Type": "application/json"}, timeout=MEDIA_TIMEOUT
            )
