...
class MemoryIntegratedAgent:
        ...
        if messages and isinstance(messages, list):
            if logger.isEnabledFor(logging.DEBUG):
                ...
                for idx, msg in enumerate(messages):
                    logger.debug(
                        f"Message {idx}: type={type(msg)}, keys={list(msg.keys()) if isinstance(msg, dict) else 'N/A'}"
                    )
                    if isinstance(msg, dict):
                        content = msg.get("content", [])
                        logger.debug(
                            f"Message {idx} content: type={type(content)}, length={len(content) if isinstance(content, list) else 'N/A'}"
                        )
                                ...
                                for block_idx, block in enumerate(content):
                                    if isinstance(block, dict) and "image" in block:
                                        ...
                                        if "source" in image_info:
                                            ...
                                            if "bytes" in source:
                                                logger.debug(
                                                    f"Image found in invoke_async: {len(source['bytes'])} bytes, format={image_info.get('format', 'unknown')}"
                                                )
                                            elif "s3Location" in source:
                                                logger.debug(
                                                    f"Image S3 URI found: {source['s3Location'].get('uri', 'unknown')}"
                                                )
                            else:
    ...
    def _parse_a2a_parts(self, parts: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {"prompt": prompt, "image_base64": image_base64, "image_s3_uri": s3_uri, "image_format": image_format}
    ...
    async def __call__(self, task_input: Optional[str] = None, **kwargs: Any) -> str:
        ...
        logger.info(
            f"Calling vision agent: prompt='{parsed_data['prompt'][:50]}...', format={parsed_data['image_format']}, has_base64={bool(parsed_data['image_base64'])}, has_s3={bool(parsed_data['image_s3_uri'])}"
        )
        ...
        for block in content_blocks:
            ...
            if "text" in block and "image" not in block and "type" not in block:
                ...
                if extracted:
                    ...
                    content.append({"image": {"format": extracted["format"], "source": {"bytes": extracted["bytes"]}}})
            ...
            if block_type == "file" and "fileUri" in block:
                ...
                if file_uri.startswith("s3://"):
                    ...
                    if mime_type.startswith("image/"):
                        content.append({"image": {"format": format_str, "source": {"s3Location": {"uri": file_uri}}}})
                    elif mime_type.startswith("video/"):
                        if format_str == "3gp":
                            format_str = "three_gp"
                        content.append({"video": {"format": format_str, "source": {"s3Location": {"uri": file_uri}}}})
            ...
            # Handle A2A DataPart (for base64 content)
                ...
                if mime_type.startswith("image/"):
                    content.append({"image": {"format": format_str, "source": {"bytes": data_bytes}}})
                elif mime_type.startswith("video/"):
                    if format_str == "3gp":
                        format_str = "three_gp"
                    content.append({"video": {"format": format_str, "source": {"bytes": data_bytes}}})
            ...
            # Handle legacy format (direct image/video parts) for backward compatibility
                ...
                if "base64" in source:
                    ...
                    content.append(
                        {"image": {"format": image_format, "source": {"s3Location": {"uri": source["s3Location"].get("uri")}}}}
                    )
                ...
                if "base64" in source:
                    ...
                    try:
                        ...
                        content.append({"video": {"format": video_format, "source": {"bytes": video_bytes}}})
                    except Exception as e:
                        logger.error(f"Failed to decode base64 video: {e}")
                elif "s3Location" in source:
                    content.append(
                        {"video": {"format": video_format, "source": {"s3Location": {"uri": source["s3Location"].get("uri")}}}}
                    )
    ...
    async def stream_async(self, content_blocks: List[Any]) -> AsyncIterator[Dict[str, Any]]:
        ...
        if logger.isEnabledFor(logging.DEBUG):
            ...
            logger.debug(
                f"content_blocks type: {type(content_blocks)}, length: {len(content_blocks) if isinstance(content_blocks, list) else 'N/A'}"
            )
        ...
        if not has_image:
            logger.debug(
                "No image found in expected format in content_blocks - will attempt extraction from text blocks if present"
            )
...
def main() -> None:
    ...
    server = A2AServer(agent=agent, port=DEFAULT_PORT, host=DEFAULT_HOST)
