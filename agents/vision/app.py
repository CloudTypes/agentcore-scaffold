...
class MemoryIntegratedAgent:
    ...
    def __init__(self, vision_agent_wrapper: VisionAgent):
        ...
        self.strands_agent = vision_agent_wrapper.strands_agent
        ...
        if not hasattr(self.strands_agent, 'description') or not self.strands_agent.description:
            self.strands_agent.description = 'Vision agent for image analysis and visual content understanding'
        ...
        self.name = getattr(self.strands_agent, 'name', 'vision-agent')
    ...
    async def stream_async(self, content_blocks):
        ...
        if hasattr(self.strands_agent, 'stream_async'):
            ...
            # This is not ideal but allows streaming to work
    ...
    async def run(self, messages, **kwargs):
        ...
        if user_message and "user_id" in kwargs and "session_id" in kwargs:
            try:
                ...
                recent = await self.vision_agent_wrapper.memory.get_recent_messages(
                ...
                relevant = await self.vision_agent_wrapper.memory.semantic_search(
                    user_id=kwargs["user_id"],
                    query=user_message,
                    limit=5
                )
