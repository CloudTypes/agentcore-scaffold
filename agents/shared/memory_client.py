...
class MemoryClient:
    ...
    async def get_recent_messages(self, user_id: str, session_id: str, limit: int = 10) -> List[Dict[str, str]]:
        """
        ...
        try:
            ...
            events = await loop.run_in_executor(
                ...
                lambda: client.list_events(
                    memory_id=self.memory_id, actor_id=sanitized_actor_id, session_id=session_id, max_results=limit
                ),
            )
            ...
            for event in events_list[:limit]:
                ...
                for msg in messages_data:
                    if isinstance(msg, tuple) and len(msg) == 2:
                        ...
                        messages.append(
                            {"role": msg.get("role", "user").lower(), "content": msg.get("content", msg.get("text", ""))}
                        )
    ...
    async def semantic_search(
        self, user_id: str, query: str, limit: int = 5, min_similarity: float = 0.7
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            response = await loop.run_in_executor(
                ...
                lambda: client.retrieve_memory_records(
                    memoryId=self.memory_id, namespace=namespace, searchCriteria={"searchQuery": query, "topK": limit}
                ),
            )
            ...
            for record in records:
                ...
                if text:
                    messages.append(
                        {"role": "assistant", "content": text}  # Semantic memories are typically from past conversations
                    )
