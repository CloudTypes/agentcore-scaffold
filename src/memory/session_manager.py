...
class MemorySessionManager:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            all_sessions = self.memory_client.list_sessions(actor_id=self.actor_id, top_k=past_sessions_count + 5)

            # Filter out current session and take the most recent N sessions
            past_sessions = [session for session in all_sessions if session.get("session_id") != self.session_id][
                :past_sessions_count
            ]
            ...
            for session in past_sessions:
                ...
                try:
                    summary_record = self.memory_client.get_session_summary(actor_id=self.actor_id, session_id=session_id)
                    ...
                    if summary_record:
                        ...
                        if summary_text:
                            session_summaries.append({"summary": summary_text, "metadata": metadata})
            ...
            logger.info(
                f"Successfully retrieved {len(session_summaries)} session summaries out of {len(past_sessions)} past sessions"
            )
            ...
            logger.info(
                f"Initialized session {self.session_id} for actor {self.actor_id} with {len(session_summaries)} past session summaries"
            )
        except Exception as e:
    ...
    def store_user_input(self, text: Optional[str] = None, audio_transcript: Optional[str] = None) -> None:
        ...
        try:
            self.memory_client.store_event(
                ...
                payload={"text": text, "audio_transcript": audio_transcript, "content": content},
            )
        except Exception as e:
    ...
    def store_tool_use(self, tool_name: str, input_data: Dict[str, Any], output_data: Dict[str, Any]) -> None:
        ...
        try:
            self.memory_client.store_event(
                ...
                session_id=self.session_id,
                event_type="tool_use",
                payload={"tool_name": tool_name, "input": input_data, "output": output_data},
            )
        except Exception as e:
