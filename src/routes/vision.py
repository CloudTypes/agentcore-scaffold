...
def get_s3_client() -> boto3.client:
    ...
    if _s3_client is None:
        try:
            ...
            raise HTTPException(status_code=503, detail=f"S3 client not available: {str(e)}")
    return _s3_client
...
def _build_media_content(
    media_type: Literal["image", "video"], format_str: str, base64_data: Optional[str], s3_uri: Optional[str]
) -> Dict[str, Any]:
    ...
    if media_type == "image":
        return {"type": "image", "image": {"format": format_str, "source": source}}
    else:  # video
        return {"type": "video", "video": {"format": format_str, "source": source}}
...
async def analyze_vision(request: VisionAnalysisRequest) -> VisionAnalysisResponse:
    ...
    if request.base64Data:
        ...
        if not request.mimeType:
            raise HTTPException(status_code=400, detail="mimeType is required when base64Data is provided")
        ...
        if not request.mimeType.startswith(expected_prefix):
            raise HTTPException(
                status_code=400, detail=f"mimeType '{request.mimeType}' does not match mediaType '{request.mediaType}'"
            )
    ...
    try:
        media_content = _build_media_content(
            media_type=request.mediaType, format_str=format_str, base64_data=request.base64Data, s3_uri=request.s3Uri
        )
    ...
    try:
        ...
        orchestrator_payload = {"message": request.prompt, "media_content": media_content, "media_type": request.mediaType}
        ...
        async with httpx.AsyncClient(timeout=120.0) as client:
            ...
            response = await client.post(
                f"{orchestrator_base}/api/vision", json=orchestrator_payload, headers={"Content-Type": "application/json"}
            )

            response.raise_for_status()
            result = response.json()

            return VisionAnalysisResponse(text=result.get("text", str(result)), usage=result.get("usage"))
