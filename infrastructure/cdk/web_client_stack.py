...
class WebClientStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        certificate_arn = self.node.try_get_context("certificate_arn") or os.getenv("CLOUDFRONT_CERTIFICATE_ARN") or None

        # Custom domain from context (optional)
        domain_name = self.node.try_get_context("domain_name") or os.getenv("CLOUDFRONT_DOMAIN_NAME") or None
        ...
        distribution = cloudfront.Distribution(
            ...
            error_responses=[
                cloudfront.ErrorResponse(
                    http_status=404, response_http_status=200, response_page_path="/index.html", ttl=cdk.Duration.minutes(5)
                ),
                cloudfront.ErrorResponse(
                    http_status=403, response_http_status=200, response_page_path="/index.html", ttl=cdk.Duration.minutes(5)
                ),
            ],
        ...
        # Outputs
        CfnOutput(
            self, "WebClientBucketName", value=self.web_bucket.bucket_name, description="S3 bucket name for web client files"
        )
        ...
        if domain_name:
            CfnOutput(self, "CustomDomainURL", value=f"https://{domain_name}", description="Custom domain URL for web client")
