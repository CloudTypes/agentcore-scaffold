    ...
    def test_multiple_text_messages_sequence(self, test_client, mock_env_vars):
        ...
        async def _mock_run(inputs=None, outputs=None):
            ...
            if outputs:
                output = outputs[0]
                await output(BidiConnectionStartEvent(
                    connection_id=f"test-conn-{call_count['count']}",
                    model="test-model"
                ))
                ...
                await output(BidiResponseCompleteEvent(
                    response_id=f"test-response-{call_count['count']}",
                    stop_reason="complete"
                ))
    ...
    def test_audio_message_roundtrip(self, test_client, mock_env_vars, sample_pcm_audio):
        ...
        async def _mock_run(inputs=None, outputs=None):
            if outputs:
                ...
                await output(BidiResponseStartEvent(response_id="test-response-1"))
                # Send audio response
                await output(BidiAudioStreamEvent(
        ...
        with patch.object(agent, 'create_nova_sonic_model') as mock_create_model, \
            ...
            try:
                with test_client.websocket_connect("/ws") as websocket:
                    ...
                    assert any(msg.get("type") == "connection_start" for msg in messages), f"Expected connection_start but received: {messages}"
                    
                    # Verify response_start
                    assert any(msg.get("type") == "response_start" for msg in messages), f"Expected response_start but received: {messages}"
    ...
    def test_database_tool_flow(self, test_client, mock_env_vars):
        ...
        async def _mock_run(inputs=None, outputs=None):
            if outputs:
                ...
                tool_event = ToolUseStreamEvent(
                    delta=ContentBlockDelta(text=""),
                    current_tool_use={"tool_name": "database_query", "parameters": {"table": "users", "filter_field": "name", "filter_value": "Alice"}}
                )
