...
def voice_client(mock_user, mock_memory_client):
    ...
    with patch('agent.memory_client', mock_memory_client), \
         patch('agent.oauth2_handler', MagicMock()):
...
class TestVoiceModeFlow:
    ...
    def test_voice_mode_initial_connection(self, voice_client, mock_memory_client):
        ...
        with patch('agent.create_nova_sonic_model'), \
            ...
            with voice_client.websocket_connect(
                f"/ws?token=test-token&session_id={session_id}"
            ) as websocket:
