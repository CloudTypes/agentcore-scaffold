...
class TestWebSocketOutput:
    ...
    async def test_audio_stream_event(self, output, websocket):
        ...
        if EVENT_CLASSES_AVAILABLE:
            ...
            event = BidiAudioStreamEvent(audio="base64_audio_data", format="pcm", sample_rate=16000, channels=1)
        else:
    ...
    async def test_transcript_stream_event_final(self, output, websocket):
        ...
        event = BidiTranscriptStreamEvent(delta=mock_delta, text="Hello, world", role="user", is_final=True)
    ...
    async def test_transcript_stream_event_not_final(self, output, websocket):
        ...
        mock_delta = MagicMock(spec=ContentBlockDelta)
        event = BidiTranscriptStreamEvent(delta=mock_delta, text="Hello, wo", role="user", is_final=False)
    ...
    async def test_response_complete_event(self, output, websocket):
        ...
        event = BidiResponseCompleteEvent(response_id="test-response-123", stop_reason="complete")
    ...
    async def test_connection_start_event(self, output, websocket):
        ...
        event = BidiConnectionStartEvent(connection_id="test-connection-123", model="amazon.nova-sonic-v1:0")
    ...
    async def test_connection_close_event(self, output, websocket):
        ...
        event = BidiConnectionCloseEvent(connection_id="test-connection-123", reason="client_disconnect")
    ...
    async def test_tool_use_event(self, output, websocket):
        ...
        event = ToolUseStreamEvent(
            delta=mock_delta, current_tool_use={"tool_name": "calculator", "content": "Some tool content"}
        )
