...
class TestCreateAgent:
    ...
    @patch.dict(os.environ, {"SYSTEM_PROMPT": "Custom system prompt for testing"}, clear=False)
    def test_agent_creation_with_custom_system_prompt(self):
        """Test agent creation uses custom system prompt from environment."""
        # Reload the module to pick up environment variable changes
        importlib.reload(agent)
        # Patch after reload
        with patch("agent.BidiAgent") as mock_agent_class, patch("agent.create_nova_sonic_model") as mock_create_model:
            mock_model = MagicMock()
            mock_create_model.return_value = mock_model
            mock_agent = MagicMock()
            mock_agent_class.return_value = mock_agent
